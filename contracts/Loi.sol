...
import "Register.sol";
 ...
 contract Loi is Register{
    ...
    event Law_Created(bytes title, bytes Description);
    ...
    constructor(address Agora_address){
        Register_Authorities.add(Agora_address);
    }
    
    
    /*Register API functions. Callable by other contracts*/
    function AddLaw(bytes memory Title, bytes memory Description) external returns(bool,bytes memory){
        //require(Lois[Title].Timestamp == 0, "Loi: Title already existing");
        if(Lois[Title].Timestamp != 0){
            return (false, bytes("Loi: Title already existing"));
        }
        ...
        emit Law_Created(Title, Description);
        return (true,bytes(""));
    }
    
    function AddArticle(bytes memory Law_Title, bytes memory Article_Title, bytes memory Content ) external returns(bool,bytes memory){
        //require(Lois[Law_Title].Timestamp != 0, "Loi: Non existing law");
        if(Lois[Law_Title].Timestamp == 0){
            return (false, bytes("Loi: Non existing law"));
        }
        bytes32 key = keccak256(abi.encode(Article_Title, Content));
        //require(Articles[key].Timestamp ==0, "Loi: Already existing article");
        if(Articles[key].Timestamp !=0){
            return (false, bytes("Loi: Already existing article"));
        }
    ...
    function Remove_Article(bytes memory Law_Title, bytes32 article) external returns(bool,bytes memory){
        if(Lois[Law_Title].Timestamp ==0 || Articles[article].Timestamp == 0){
            return (false, bytes("Loi: Non existing article"));
        }
        if(!Lois[Law_Title].List_Articles.contains(article)){
            return (false, bytes("Article no exist in this law"));
        }
    ...
    function Change_Law_Description(bytes memory law, bytes memory new_Description)external returns(bool,bytes memory){
        if(Lois[law].Timestamp ==0){
            return (false, bytes("Loi: Law doesn't exist"));
        }
